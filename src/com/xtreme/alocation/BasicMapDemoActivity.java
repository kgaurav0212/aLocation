...
public class BasicMapDemoActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().getExtras() != null)
			selectedIds = getIntent().getIntegerArrayListExtra("selectedIds");
		setUpMapIfNeeded();
	...
	private void setUpMap() {
		...
		// locationList.get(0).getPosition().toString(), Toast.LENGTH_SHORT);
		// toast.show();
	...
	private void showMarkers() {
		...
		for (int i = 0; i < selectedIds.size(); i++) {
			...
			MarkerOptions markerOptions = new MarkerOptions().position(
					locationList.get(index).getPosition()).title(
					locationList.get(index).getName());
		...
		mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(locationList.get(1)
				.getPosition(), 5));
